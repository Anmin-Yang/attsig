...
subjs = raw(201:end,1);%242 failed, 231x excluded 
%selection = raw(2:end,4); this selection for head motion
behavior = raw(201:end,5);
...
for i = 1:length(subjs)
    ...
    if behavior{i} == 1
        ...
        %discard first 9 volume
        i=1;
        for j = 1:158
            funcName{i} =  ['/nfs/e2/workingshop/tianjinhua/NSP_attention/nspSpm/',subj,'/sa/func.nii,',num2str(j)];
            i = i+1;
        end

        %funcName = funcName(5:164);

        funcPath = ['/nfs/e2/workingshop/tianjinhua/NSP_attention/nspSpm/',subj,'/sa/'];
        ...
        matlabbatch{9}.spm.spatial.smooth.prefix = 's8';
