...
for i in range(boot_time):
    temp_dic = np.load(os.path.join(boot_path, f'fa_boot_{i}.npy'),
                        allow_pickle=True).item()
...
for alpha_level in alpha_pool:
    ...
    np.save(os.path.join(mask_path, f'fa_{alpha_level}.npy'),
            voxel_mask)
...
np.save(os.path.join(mask_path, 'fa_fdr.npy'), mask_fdr)

## FWE correction 
# Bonferroni 
mask_fwe = multipletests(p_val,alpha=0.05,method='bonferroni')[0]
print(f'passed voxel num: {np.sum(mask_fwe)}')
np.save(os.path.join(mask_path, 'fa_bonferroni.npy'), mask_fwe)
